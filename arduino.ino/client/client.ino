...
const char* ssid = "PLDTHOMEFIBR5GB5F85";
const char* password = "parianFamily143@";
const char* device_token  = "d5f089af119e2655";
String URL = "http://192.168.1.37/adsms/getdata.php";
String getData, Link;
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------//
...
void loop() {
  ...
  digitalWrite(ON_Board_LED, LOW);
    ...
    //Post Data
    getData =  "?card_uid=" + String(UIDresultSend) + "&device_token=" + String(device_token); // Add the Card ID to the GET array in order to send it
    Link = URL + getData;
  ...
  digitalWrite(ON_Board_LED, HIGH);
    ...
    HTTPClient http;
   getData =  "?door="; // Add the Card ID to the GET array in order to send it
    Link = URL + getData;
    http.begin(Link);  //Specify request destination
    http.addHeader("Content-Type", "application/x-www-form-urlencoded"); //Specify content-type header
...
void handleSentVar() {
  ...
  state         = root["sensor_reading"].as<int>();
  handleLIGHT();
